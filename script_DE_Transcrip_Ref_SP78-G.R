setwd("D:/Helba/Salmon/SP78/SP78-G")
caminho_pasta <- "D:/Helba/Salmon/SP78/SP78-G"
arquivos <- list.files(path = caminho_pasta, pattern = "*.sf", full.names = TRUE)
tx2gene <- read.table("D:/Helba/Salmon/SP78/SP78-G/tx2gene.csv", header = TRUE)
...
res <- results (dds)
write.csv(res, "D:/Helba/Salmon/SP78/SP78-G/SP78-G_DESeq.csv")
...
resultado <- read_csv("D:/Helba/Salmon/SP78/SP78-G/SP78-G_DESeq.csv")
...
ggplot(resultado, aes(x = log2FoldChange, y = -log10(pvalue), color = Regulation)) + geom_point(alpha = 0.5) + 
  ...
  labs(title = "Differentially Expressed contigs - MO4 inoculation in Susceptible Cultivar 03", x = "Log2 Fold Change", y = "-Log10 p-value")+
  theme(
