...
caminho_pasta <- "D:/Helba/Salmon/CB49"
arquivos <- list.files(path = caminho_pasta, pattern = "*.sf", full.names = TRUE)
tx2gene <- read.table("D:/Helba/Salmon/CB49/tx2gene.csv", header = TRUE)
...
res <- results (dds)
write.csv(res, "D:/Helba/Salmon/CB49/CB49-L_DESeq.csv")
...
resultado <- read_csv("D:/Helba/Salmon/CB49/CB49-L_DESeq.csv")
...
ggplot(resultado, aes(x = log2FoldChange, y = -log10(pvalue), color = Regulation)) + geom_point(alpha = 0.5) + 
  ...
  labs(title = "Differentially Expressed contigs - MO2 inoculation in Susceptible Cultivar 02", x = "Log2 Fold Change", y = "-Log10 p-value")+
  theme(
